...
const UpdateAdvertisement = ({ match }) => {
    ...
    const init = (advertisementId) => {
        getAdvertisement(advertisementId).then((advertisement) => {
            if(advertisement.error) {
                ...
                const startDate = advertisement.adStartDate.split('T')[0];
                const endDate = advertisement.adEndDate.split('T')[0];
    ...
    const showSuccess = () => success && (
        <Alert severity="success">
            ...
            Advertisement succesfully updated — <strong><Link to="/admin/manageAds">Go back</Link></strong>
        </Alert>
    );
